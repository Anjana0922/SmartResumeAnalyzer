...
export default function AtsTemplate({ resume }) {
  ...
  // Contact line items (ensuring no duplicated name in location)
  ...
  return (
    <div className="resume-print-root bg-white text-black font-sans px-8 py-7 w-[794px] mx-auto text-xs leading-relaxed box-border">
      {/* Header - Centered & Clean (No Photo for ATS) */}
      <header className="text-center pb-2.5 mb-3.5 border-b border-black resume-section-block pdf-avoid-break">
        ...
        {contactItems.length > 0 && (
          <p className="text-xs text-black mt-1 space-x-1.5">
      ...
      {/* Professional Summary (Only if present) */}
      {summary && (
        <section className="mb-3.5 resume-section-block">
          <h2 className="text-xs font-bold uppercase tracking-wider text-black border-b border-black pb-0.5 mb-1 resume-section-header pdf-avoid-break">
              ...
              <div key={idx} className="resume-entry pdf-avoid-break">
                <div className="flex flex-row items-baseline justify-between font-bold text-black">
                  <span>
                    ...
                    {edu.degree && edu.institution && <span> — {edu.institution}</span>}
                    {edu.board && <span> (Board: {edu.board})</span>}
                  </span>
                  <span className="shrink-0">{edu.year || ""}</span>
                </div>
                {edu.location && <p className="text-xs italic text-black">{edu.location}</p>}
                {edu.score && <p className="text-xs text-black">Score/Grade: {edu.score}</p>}
                ...
                {edu.additional_details && (
                  <p className="text-xs text-black mt-0.5">{edu.additional_details}</p>
                )}
              </div>
            ))}
          </div>
      ...
      {/* Skills & Competencies (Only if present) */}
      ...
      {/* Work Experience / History (Only if present) */}
      {experience.length > 0 && (
        <section className="mb-3.5 resume-section-block">
          <h2 className="text-xs font-bold uppercase tracking-wider text-black border-b border-black pb-0.5 mb-1.5 resume-section-header pdf-avoid-break">
            ...
            {experience.map((exp, idx) => {
              ...
              return (
                <div key={idx} className="resume-entry pdf-avoid-break">
                  ...
                  {exp.description && (
                    <p className="text-black mt-0.5 text-justify">
                      {exp.description}
                    </p>
                  )}
                  ...
                  {exp.achievements && (
                    <p className="text-xs mt-0.5">
                      <strong>Key Achievement:</strong> {Array.isArray(exp.achievements) ? exp.achievements.join("; ") : exp.achievements}
                    </p>
                  )}
            ...
            {projects.map((proj, idx) => (
              <div key={idx} className="resume-entry pdf-avoid-break">
                ...
                {Array.isArray(proj.highlights) && proj.highlights.length > 0 && (
                  <ul className="list-disc list-inside mt-0.5 space-y-0.5">
                    {proj.highlights.filter(Boolean).map((hl, hIdx) => (
                      <li key={hIdx}>{hl}</li>
                    ))}
                  </ul>
                )}
      ...
      {/* Certifications & Honors (Only if present) */}
        ...
        <section className="mb-3.5 resume-section-block">
          ...
          <div className="space-y-2">
            {certificates.map((cert, idx) => (
              <div key={`cert-${idx}`} className="resume-entry pdf-avoid-break">
                <div className="flex flex-row items-baseline justify-between font-bold text-black">
                  <span>
                    {cert.name}
                    {cert.issuer && <span className="font-normal text-xs"> — {cert.issuer}</span>}
                  </span>
                  <span className="text-xs font-normal shrink-0">{cert.year || ""}</span>
                </div>
                {cert.description && (
                  <p className="text-black text-justify mt-0.5">
                    {cert.description}
            ...
            {achievements.length > 0 && (
              <ul className="list-disc list-inside space-y-0.5 text-black mt-1">
      ...
      {/* Custom & Unfamiliar Sections (Preserved completely) */}
          ...
          {customSections.map((sec, sIdx) => (
            <section key={sIdx} className="resume-section-block">
              ...
              {Array.isArray(sec.items) && sec.items.length > 0 ? (
                ...
                <p className="text-black whitespace-pre-line text-xs">{sec.rawText}</p>
              )}
            </section>
          ))}
        </div>
